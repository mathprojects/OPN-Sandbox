...
(* Define neither MPIR nor GMP if you want to support both and give up some
   function that are not availble in both *)
(* Define MPIR if you only want to support MPIR *)
{ $DEFINE MPIR}
(* Define GMP if you only want to support GMP *)
{$DEFINE GMP}
...
(* Modifications by Matthew Thomas
   5/17/2020 - IsCongruentToCModD added
   5/19/2020 - Conditional defines MPIR and GMP added.
               MPIR functions not available in GMP are not compiled if MPIR not defined.
               GMP function not available in MPIR were added, and are not compiled if GMP not defined.
               This affects gmpKronecker, gmpProbablePrime, gmpNextLikelyPrime,
               gmpRandomFloatDebug
*)
...
type
   ...
   public
      ...
      function IsCongruentToCModD(const C: gmpInteger; const D: gmpInteger): boolean; overload;
      function IsCongruentToCModD(const C: Cardinal; const D: Cardinal): boolean; overload;
      ...
      //Trunc and Round cannot be overloaded in FPC
      //fpc class operator Trunc(const Value: gmpInteger): gmpInteger;
      //fpc class operator Round(const Value: gmpInteger): gmpInteger;
   ...
   public
      ...
      //Trunc cannot be overloaded in FPC
...
{$ENDIF}
{$IFDEF GMP}
   function gmpProbablePrime(const N: gmpInteger; Reps: Integer ): Boolean;
   function gmpNextLikelyPrime(const N: gmpInteger): gmpInteger; // Returns the next likely prime greater than N.
{$ENDIF}
   // For float
...
function gmpInteger.IsCongruentToCModD(const C: gmpInteger; const D: gmpInteger): boolean;
begin
  result := (mpz_congruent_p(NativeObject^, C.NativeObject^, D.NativeObject^)) <> 0;
  C.ReleaseTemporary;
  D.ReleaseTemporary;
end;

function gmpInteger.IsCongruentToCModD(const C: Cardinal; const D: Cardinal): boolean;
begin
  result := (mpz_congruent_ui_p(NativeObject^, C, D) <> 0);
end;
...
function gmpProbablePrime(const N: gmpInteger; Reps: Integer ): Boolean;
begin
   Result := mpz_probab_prime_p(N.NativeObject^, Reps) = 1;
end;

function gmpNextLikelyPrime(const N: gmpInteger): gmpInteger;
begin
   Result := GetTemporaryInteger()^;
   mpz_nextprime(Result.NativeObject^, N.NativeObject^);
   N.ReleaseTemporary;
end;
{$ENDIF}
